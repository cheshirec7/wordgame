...
FrozenTrieNode.prototype = {
    ...
    getChildLetters: function () {
        for (var a, b = [], c = 0; c < this.childCount; c++) {
            a = this.getChild(c);
            b.push(a.letter);
        }
        return b;
...
FrozenTrie.prototype = {
    ...
    getNodeForString: function (a) {
        if (!m[a]) {
            for (var b = this.getRoot(), c = 0; c < a.length; c++) {
                for (var d, e = 0; e < b.getChildCount() && (d = b.getChild(e), d.letter !== a[c]); e++) ;
                if (e === b.getChildCount()) return !1;
                b = d
            }
            m[a] = b
        }
    ...
    lookup2: function (a) {
        for (var b = this.getRoot(), c = 0; c < a.length; c++) {
            for (var d, e = 0; e < b.getChildCount() && (d = b.getChild(e),
            d.letter !== a[c]); e++) ;
            if (e === b.getChildCount()) return {isWord: !1, isPrefix: !1};
            b = d
        }
        return {
            isWord: b.final, isPrefix: b.getChildCount(), nextValid: b.getChildLetters()
        }
    }, nextForks: function (a, b, c, d) {
        for (var e, f, g = a, h = 0; h < b.getChildCount(); h++)
            for (f = g, e = b.getChild(h); e;) {
                if (f += e.letter, e.final && d.push(f), 1 != e.getChildCount()) {
                    if (e.getChildCount() > 1) {
                        c.push({prefix: f, node: e});
                        break
                    }
                    break
                }
    ...
    }, getPossibilities: function (a, b) {
        if (!a || !a.length) return [];
        var c = this.getNodeForString(a);
        if (!c) return [];
        b = b || 10;
        var d, e = [], f = [{prefix: a, node: c}], g = [];
        for (c.final && (g = [a]); e.length < b;) {
            for (; g.length && e.length < b;)
                e.push(g.shift());
            if (0 === f.length || e.length === b)
                return e;
            d = f.shift();
            this.nextForks(d.prefix, d.node, f, g);
        }
    }
